...
class WordSearchGUI:
    ...
    def load_game_dialog(self):
        ...
        if not saves:
            messagebox.showinfo(self.lang.get('info'), self.lang.get('no_saves'))
            return
        
        dialog = tk.Toplevel(self.root)
        dialog.title(self.lang.get('load_title'))
        dialog.geometry("500x400")
        dialog.configure(bg=self.COLOR_BG)
        
        tk.Label(
            dialog,
            text=self.lang.get('saves_available'),
            font=("Arial", 16, "bold"),
            bg=self.COLOR_BG,
            fg="#ECF0F1"
        ).pack(pady=20)
        ...
        for save in saves:
            listbox.insert(tk.END, f"{save['name']} - {self.lang.get('level')} {save['level']} - {save['timestamp'][:19]}")
    ...
    def save_game_dialog(self):
        ...
        save_name = simpledialog.askstring(self.lang.get('save'), self.lang.get('save_dialog'))
        if save_name:
            self.save_manager.save_game(self.game.get_game_state(), save_name)
            messagebox.showinfo(self.lang.get('success'), f"{self.lang.get('save_success')} '{save_name}'!")
    ...
    def show_game_screen(self):
        ...
        self.level_label = tk.Label(
            header,
            text=f"{self.lang.get('level')} {self.game.current_level.number}",
            font=("Arial", 20, "bold"),
            bg=self.COLOR_BG,
            fg="#ECF0F1"
        )
        ...
        seed_label = tk.Label(
            header,
            text=f"{self.lang.get('seed')}: {self.game.seed}",
            font=("Arial", 12),
            bg=self.COLOR_BG,
            fg="#95A5A6"
        )
        ...
        tk.Label(
            words_frame,
            text=self.lang.get('words_to_find'),
            font=("Arial", 16, "bold"),
    ...
    def update_score(self):
        ...
        self.score_label.config(text=f"🎯 {self.lang.get('score')}: {score}")
    ...
    def level_complete(self):
        ...
        msg = f"🎉 {self.lang.get('congratulations')} 🎉\n\n"
        msg += self.lang.get('level_completed', level) + "\n\n"
        msg += self.lang.get('final_score', score) + "\n"
    ...
    def game_over(self):
        ...
        msg = f"⏱️ {self.lang.get('time_over')} ⏱️\n\n"
        msg += self.lang.get('words_found', found, total) + "\n"
        msg += f"{self.lang.get('score')}: {score}\n"
        msg += f"{self.lang.get('seed')}: {self.game.seed}"
        
        result = messagebox.askquestion(
            self.lang.get('time_up'),
            msg + "\n\n" + self.lang.get('replay_level')
        )
    ...
    def show_settings(self):
        ...
        dialog = tk.Toplevel(self.root)
        dialog.title(self.lang.get('settings'))
